...
const repRevenue = computed(() => calcs.value.reduce((a, c) => a + c.grandTotal, 0))
const repCost = computed(() => calcs.value.reduce((a, c) => a + c.totalCost, 0))
const repAfterDisc = computed(() => calcs.value.reduce((a, c) => a + (c.subtotal - c.discountAmount), 0))
...
const repOrders = computed(() => {
  return orders.value.map((o, i) => {
    ...
    return {
      group: o.group, no: o.no, dest: o.dest, revenueF: fmt(c.grandTotal), costF: fmt(c.totalCost),
...
const chartSeries = computed(() => catBreakdown.value.map(c => c.amt))
...
const doExportPdf = async () => {
  ...
  try {
    const blob = await dashboardService.exportPdf(getExportParams());
...
const doExportExcel = async () => {
  ...
  try {
    const blob = await dashboardService.exportExcel(getExportParams());
...
<template>
  <div class="p-mobile" style="padding:30px 32px;">
    ...
    <div style="display:flex; justify-content: space-between; align-items: center; margin-bottom: 22px; flex-wrap: wrap; gap: 14px;">
      ...
      <div style="display:flex; gap: 10px; align-items:center;">
        <div style="position:relative;">
          <input ref="drpInput" type="text" placeholder="Filter Tanggal..." style="padding:9px 36px 9px 14px;border:1px solid #d8dce4;border-radius:9px;font-size:13px;outline:none;width:220px;background:#fff;cursor:pointer;" readonly />
    ...
    <div class="grid-cols-1-mobile" style="display:grid;grid-template-columns:1.25fr .9fr;gap:18px;">
      ...
      <div style="background:#fff;border:1px solid #e8e9ee;border-radius:16px;overflow:hidden;">
        <div style="padding:18px 22px;border-bottom:1px solid #eef0f3;display:flex;justify-content:space-between;align-items:center;">
        ...
        <div style="padding:18px 22px;">
          <div style="margin-bottom: 24px; display:flex; justify-content:center;">
            <VueApexCharts :options="chartOptions" :series="chartSeries" type="donut" width="300" />
          </div>
